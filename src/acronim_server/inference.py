...
async def get_hidden_states_for_training_samples(
    ...
    try:
        ...
        yield new_log_event(logger, f"Saved hidden state for {len(hook_data)} relevant images to {sampled_hidden_states_full_saved_path}")
        yield new_log_event(logger, f"Extracted hidden states for {len(hook_data)} relevant samples wrt token={token_of_interest} in time={elapsed_time:.6f}s'")
